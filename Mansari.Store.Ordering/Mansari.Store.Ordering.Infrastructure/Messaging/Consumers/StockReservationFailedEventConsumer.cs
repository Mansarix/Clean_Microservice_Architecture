...
public sealed class StockReservationFailedEventConsumer : BackgroundService
{
    ...
    {
        StockReservationFailedEvent? integrationEvent;

        try
        {
            var json = Encoding.UTF8.GetString(eventArgs.Body.ToArray());

            integrationEvent = JsonSerializer.Deserialize<StockReservationFailedEvent>(
                json,
        ...
        if (inboxMessage is null)
        {
            inboxMessage = new InboxMessage
            {
                ...
                Type = nameof(StockReservationFailedEvent),
